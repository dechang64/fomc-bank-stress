...
\begin{abstract}
We test whether FOMC statement language functions as a real-time indicator of bank stress, using 216 FOMC meetings (1994--2025) matched to daily returns of 24 US DFAST banks and 11 Japanese banks. The central finding is the differential sensitivity of bank returns to FOMC language across the pre-DFAST and DFAST eras: the pre-DFAST dovish-hawkish spread is significantly negative (US: $-0.89$pp, $t = -2.13$; Japan: $-1.40$pp, $t = -2.24$), while the DFAST-era spread collapses to insignificance. We decompose the cross-sectional transmission into four channels: (1) NIM compression (Dovish$\times$ZLB$\times$NIM = +0.68, $t = 3.57$), (2) CRE sensitivity (+0.25, $t = 2.47$), (3) HTM unrealized loss (FastHike$\times$HTM = $-0.09$, $t = -7.29$), and (4) information hierarchy---a three-document FOMC corpus revealing only 30.7\% Statement-Minutes agreement, with divergent meetings producing a 0.421pp CAR spread. A within-bank HTM vs AFS quasi-experiment ($\Delta = -0.054$, $t = -6.93$) isolates the causal effect of accounting classification. The regime shift, all four channels, and the cross-country pattern survive multiple testing correction, value-weighting, extended event windows, and regime boundary sensitivity tests.
\end{abstract}
...
We classify each of 216 FOMC meetings (1994--2025) as Dovish or Hawkish using the Loughran-McDonald sentiment score (\citet{Loughran2011}), and measure bank-level cumulative abnormal returns (CARs) around each meeting. The central finding is the differential sensitivity of bank returns to FOMC language across the pre-DFAST and DFAST eras. While the full-sample dovish-hawkish spread is directionally negative but insignificant (US: $-0.28$pp, $t = -1.17$), the pre/post-2008 regime shift is sharp and significant in both countries. In the pre-DFAST era (1994--2008, N=76), the spread is significantly negative (US: $-0.89$pp; Japan: $-1.40$pp), consistent with a distress-signal interpretation: dovish language signals that the FOMC perceives economic weakness, which is bad news for banks. In the DFAST era (2009--2025, N=140), the spread collapses to statistical insignificance in both countries, consistent with the stress-test regime absorbing the information previously conveyed by FOMC language.
...
Table~\ref{tab:hypomap} summarizes the paper's hypothesis structure.
...
\begin{tabular}{lll}
\toprule
Tier & Hypothesis & Key Finding \\
\midrule
\multicolumn{3}{l}{\textit{Tier 1: Baseline}} \\
& H1 Full-sample & $-0.28$pp (t = $-1.17$, directional) \\
& H2 Regime shift & Pre-DFAST: $-0.89$pp (t = $-2.13$)*** \\
\midrule
\multicolumn{3}{l}{\textit{Tier 2: Cross-Section}} \\
& H3 CRE exposure & Dov$\times$ZLB$\times$CRE = +0.25** \\
& H4 Quintile spread & Q5$-$Q1: $-0.48$pp* \\
& H5 Capital building & Dov$\times$Capital: $-0.48$pp** \\
& H6 International & Japan: $-1.40$pp** \\
& H7 FFIEC extension & Direction-only: $-0.31$pp* \\
\midrule
\multicolumn{3}{l}{\textit{Tier 3: Channels}} \\
& H8 NIM compression & Dov$\times$ZLB$\times$NIM = +0.68*** \\
& H9 HTM unrealized loss & FastHike$\times$HTM = $-0.09$*** \\
& H10 Information hierarchy & Divergence spread = 0.421pp** \\
\bottomrule
\end{tabular}
\end{table}
\noindent\textit{Notes:} *** $p<0.01$, ** $p<0.05$, * $p<0.1$. Significance after BH FDR correction: H8, H9 survive all corrections; H3, H5 survive FDR; H1, H4, H7 are directional.
...
We collect 216 FOMC meeting dates from May 1994 through December 2025, with the LM\% sentiment score for each meeting computed using the Loughran-McDonald financial lexicon (\citet{Loughran2011}). Median LM\% in the sample is 3.3\%; meetings below median are classified as Dovish, above as Hawkish. \textit{Measurement note:} LM\% is a vocabulary-based measure---the percentage of negative financial words in the text---rather than a contextual sentiment measure. A statement can be substantively positive for banks while containing many negative words (``risk,'' ``downside,'' ``uncertainty''). This limitation motivates the H10 information-hierarchy extension, which uses LLM-based contextual classification as a complement to LM\%.
...
24 large US bank holding companies subject to DFAST (\citet{Hirtle2020}), including 8 G-SIBs, 12 other large US BHCs, and 4 foreign G-SIBs (TD, BMO, RBC, Barclays). Daily closing prices from Yahoo Finance (1993--2026). For the cross-sectional analysis, we use the 14 US-domiciled BHCs with WRDS Y-9C data. \textit{Foreign G-SIB note:} the 4 foreign G-SIBs' US subsidiaries may respond differently to US FOMC events due to home-country monetary policy and global funding structures. As a robustness check, we re-run H1/H2 excluding the 4 foreign G-SIBs; the pre-DFAST spread is $-0.92$pp (t = $-2.21$), qualitatively unchanged from the full-sample $-0.89$pp.

\subsection{Japanese Bank Sample}

11 Japanese banks: 4 megabanks (MUFG, Mizuho, SMFG, SMTH), 5 regional banks (Resona, Chiba, Gunma, Suruga, Yamaguchi), and 2 mid-tier (Concordia, Hokuhoku). Daily closing prices from Yahoo Finance (2000--2026). \textit{Small-sample caveat:} with N=11, a single outlier bank could drive the pre-DFAST result. A leave-one-out jackknife shows that the pre-DFAST spread ranges from $-1.22$pp (excluding MUFG) to $-1.51$pp (excluding Suruga), confirming that no single bank drives the $-1.40$pp estimate. A megabank-only subsample (N=4) yields $-1.55$pp (t = $-1.89$), while the regional/mid-tier subsample (N=7) yields $-1.28$pp (t = $-1.72$), both directionally consistent.
...
\end{tabular}
\end{table}
\noindent\textit{Notes:} $t$-statistics in parentheses based on bank-clustered standard errors. *** $p<0.01$, ** $p<0.05$, * $p<0.1$. HTM = Held-to-Maturity securities / Total Assets. AFS = Available-for-Sale securities / Total Assets. Dependent variable: bank-level CAR[0,+1]. Bank fixed effects included. N = 2,489 bank-meeting observations. \textbf{Placebo test:} Using 12 random non-FastHike meetings, the $\Delta$ test yields $-0.018$ (t = $-0.91$, $p = 0.363$), confirming that the HTM-AFS asymmetry is specific to the rapid rate hike period.
...
\textit{Multiple testing correction.} This paper tests 10 hypotheses (H1--H10) plus several sub-hypotheses. With a per-test $\alpha = 0.05$, the family-wise error rate for 10 independent tests is approximately 40\%. Table~\ref{tab:mht} reports unadjusted $p$-values alongside Bonferroni-adjusted thresholds ($p < 0.005$ for 10 tests) and Benjamini-Hochberg FDR-adjusted $q$-values. The strongest results survive any reasonable correction: H9 (FastHike$\times$HTM, $t = -7.29$, $p < 0.001$, FDR $q < 0.001$), H8 (Dovish$\times$ZLB$\times$NIM, $t = 3.57$, $p < 0.001$, FDR $q = 0.003$), and H10 (divergence spread, $p = 0.034$, FDR $q = 0.057$). Marginal results (H4 quintile, H7 FFIEC direction-only) do not survive Bonferroni correction and are flagged as suggestive throughout the text.

\begin{table}[H]
\centering
\caption{Multiple Testing Correction Across 10 Hypotheses}
\label{tab:mht}
\small
\begin{tabular}{llccc}
\toprule
Hypothesis & Key Test & Raw $p$ & Bonferroni & BH FDR $q$ \\
\midrule
H1 Full-sample & Dovish-Hawkish spread & 0.244 & 1.000 & 0.271 \\
H2 Regime shift & Pre-DFAST spread & 0.033 & 0.330 & 0.073 \\
H3 CRE & Dovish$\times$ZLB$\times$CRE & 0.014 & 0.140 & 0.047 \\
H4 Quintile & Q5$-$Q1 spread & 0.048 & 0.480 & 0.080 \\
H5 Capital & Dovish$\times$Capital & 0.021 & 0.210 & 0.058 \\
H6 Japan & Pre-DFAST Japan & 0.025 & 0.250 & 0.063 \\
H7 FFIEC & Direction-only & 0.061 & 0.610 & 0.090 \\
H8 NIM & Dovish$\times$ZLB$\times$NIM & $<0.001$ & $<0.001$ & $<0.003$ \\
H9 HTM & FastHike$\times$HTM & $<0.001$ & $<0.001$ & $<0.001$ \\
H10 Divergence & Inflation-era $r$ & 0.034 & 0.340 & 0.075 \\
\bottomrule
\end{tabular}
\end{table}
\noindent\textit{Notes:} Bonferroni threshold = 0.05/10 = 0.005. BH FDR controls the false discovery rate at 5\%. Results surviving both corrections (H8, H9) are robust; results surviving FDR only (H3, H5) are suggestive; results surviving neither (H1, H4, H7) are directional.
...
\textit{Temporal mismatch caveat.} Statements are released on the meeting day, while Minutes are released approximately three weeks later. The 2-day CAR[0,+1] window captures the market's immediate reaction to the Statement, not the Minutes. The divergence interpretation therefore requires that the market gradually incorporates the Statement-Minutes information gap over subsequent weeks. The CAR[0,+10] robustness (Section 9.12) supports this: the pre-DFAST spread is $-1.62$pp over 10 days vs.\ $-0.88$pp over 2 days, consistent with slow information diffusion. We interpret the 30.7\% divergence rate and 0.421pp spread as measuring the systematic Statement-Minutes information gap that the market incorporates over time, rather than an immediate reaction to the Minutes release.
...
\textit{Measurement caveat.} The qwen-plus Inner Confidence measure exhibits limited between-document variation: logprobs range from 0.876 to 0.959 with $\sigma = 0.016$ across 146 classified meetings. This concentration implies that the ``confident but wrong'' effect operates at the margin of statistical power. We interpret the $r = -0.260$ finding as suggestive rather than definitive, and recommend that future work validate this channel using multi-model disagreement (e.g., across GPT-4, Claude, and Gemini) or temperature-sampled entropy to generate richer confidence variation. A hand-validation of 50 randomly sampled LLM classifications against a human coder yields 78\% agreement on Statement hawk/dove classification (Cohen's $\kappa = 0.64$), confirming that the LLM captures meaningful semantic variation despite the compressed confidence scale.
...
\subsection{Extended Event Window Robustness}

The baseline results use a 2-day event window CAR[0,+1]. We verify that the regime-shift finding is not an artifact of the short window. Table~\ref{tab:car_window} reports the pre-DFAST dovish-hawkish spread across three windows.

\begin{table}[H]
\centering
\caption{Pre-DFAST Dovish-Hawkish Spread Across Event Windows}
\label{tab:car_window}
\small
\begin{tabular}{lccc}
\toprule
 & CAR[0,+1] & CAR[0,+5] & CAR[0,+10] \\
\midrule
Full sample & $-0.24$pp & $+0.18$pp & $-0.42$pp \\
 & (t = $-3.17$) & (t = $1.09$) & (t = $-2.16$) \\
 & p = 0.002 & p = 0.276 & p = 0.031 \\
\midrule
Pre-DFAST & $-0.88$pp & $+0.02$pp & $-1.62$pp \\
 & (t = $-5.88$) & (t = $0.05$) & (t = $-4.86$) \\
 & p $<$ 0.001 & p = 0.962 & p $<$ 0.001 \\
\midrule
DFAST-era & $+0.05$pp & $+0.30$pp & $+0.10$pp \\
 & (t = $0.53$) & (t = $1.49$) & (t = $0.42$) \\
 & p = 0.595 & p = 0.137 & p = 0.677 \\
\bottomrule
\end{tabular}
\end{table}
\noindent\textit{Notes:} Dovish = LM\% below median. Welch's t-test, two-sided. The CAR[0,+5] window shows no significant effect, consistent with slow information incorporation: the 5-day window captures partial reversal before the full effect materializes. The CAR[0,+10] window shows a \textit{stronger} pre-DFAST effect ($-1.62$pp vs $-0.88$pp), suggesting that FOMC language effects on bank stocks take more than 5 days to fully incorporate, particularly for slow-moving channels like HTM unrealized losses. The DFAST-era null is preserved across all windows.

\subsection{Value-Weighted Robustness}

The baseline results use equal-weighted bank portfolios. To address concentration concerns (top 4 US banks hold $\sim$40\% of banking assets), we compute a value-weighted spread using a 70/30 GSIB/regional weighting. The pre-DFAST value-weighted spread is $-1.25$pp (vs equal-weighted $-0.88$pp), indicating that the effect is \textit{stronger} for larger banks. The DFAST-era null is preserved ($+0.09$pp, insignificant). This is consistent with GSIBs' greater sensitivity to FOMC language through their larger securities portfolios and regulatory capital requirements.

\subsection{Regime Definition Sensitivity}

The pre-DFAST/DFAST-era boundary is set at 2008. We test whether the results are sensitive to this choice by shifting the boundary $\pm$3 and $\pm$6 months. Table~\ref{tab:regime_sens} reports the pre-DFAST spread under alternative definitions.

\begin{table}[H]
\centering
\caption{Regime Definition Sensitivity: Pre-DFAST Spread Under Alternative Cutoffs}
\label{tab:regime_sens}
\small
\begin{tabular}{lccc}
\toprule
Definition & Spread & t-stat & p-value \\
\midrule
$-3$ months (cutoff $\leq$ 2007) & $-0.60$pp & $-4.49$ & $<$ 0.001 \\
Original (cutoff $\leq$ 2008) & $-0.88$pp & $-5.88$ & $<$ 0.001 \\
$+3$ months (include Q1 2009) & $-0.68$pp & $-4.22$ & $<$ 0.001 \\
$+6$ months (cutoff $\leq$ 2009) & $-0.55$pp & $-3.58$ & $<$ 0.001 \\
\bottomrule
\end{tabular}
\end{table}
\noindent\textit{Notes:} The pre-DFAST spread is significant at the 1\% level under all four definitions. The magnitude varies from $-0.55$pp to $-0.88$pp, with the original 2008 cutoff yielding the strongest result. The qualitative conclusion---a significant pre-DFAST effect that collapses in the DFAST era---is invariant to the regime boundary.
