...
216 FOMC meeting dates from May 1994 through December 2025, with the LM\% sentiment score for each meeting computed using the Loughran-McDonald financial lexicon (\citet{Loughran2011}). Median LM\% in the sample is 3.3\%; meetings below median are classified as Dovish, above as Hawkish.
...
\begin{table}[H]
\centering
\caption{Comparison of Signal Divergence Measures for FOMC Stance Classification}
\label{tab:japan}
\small
\begin{tabular}{lllll}
\toprule
...
\end{tabular}
\end{table}
\noindent\textit{Notes:} Comparison of five methods for measuring FOMC signal divergence. Stance distance uses discrete categories (0 = agree, 1 = partial disagree, 2 = full disagree). ``Best'' indicates the measure with the strongest predictive power for bank CAR. LLM = qwen-plus.
