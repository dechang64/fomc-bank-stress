#!/usr/bin/env python3
"""Generate Words Beyond the Rate v12.1 — full paper with figures"""
...
def T(headers, rows, caption=None):
    ...
    return t
def FIG(path, caption, width=Inches(6.0)):
    ...
    cap.runs[0].font.size = Pt(10); cap.runs[0].italic = True
...
r2 = p2.add_run('Academy of AI, Xi\'an Jiaotong-Liverpool University, Suzhou, China')
r2.font.size = Pt(11); r2.italic = True
p3 = doc.add_paragraph(); p3.alignment = WD_ALIGN_PARAGRAPH.CENTER
r3 = p3.add_run('v12.1 — Revised Draft (June 2026)')
r3.font.size = Pt(10); r3.font.color.rgb = RGBColor(128, 128, 128)
doc.add_page_break()
...
P('A critical methodological finding motivates this revision. The standard LM dictionary (Loughran and McDonald, 2011) contains 347 positive and 2,345 negative word categories. However, prior applications to FOMC statements have used abbreviated versions with as few as 116 positive and 213 negative terms. This abbreviation introduces a severe positivity bias: 95% of FOMC statements receive positive LM% scores, compared to 76% with the full dictionary. The correlation between the abbreviated and full-dictionary LM% is −0.27 — they point in opposite directions (Figure 1).')
P('This bias has substantive implications. In the Kuttner surprise horse race regression (controlling for the federal funds rate surprise), the abbreviated LM% produces t = 8.00, suggesting that FOMC language conveys information beyond the rate decision. The full-dictionary LM% produces t = −0.22, indicating no incremental information. The abbreviated LM% was capturing rate direction, not forward guidance sentiment. This finding underscores the importance of using complete, domain-appropriate dictionaries in monetary policy text analysis.')

H('1.2 The Regime-Dependence Puzzle')
P('The full-sample regression yields negligible R² (≤3.1%) regardless of the sentiment measure. However, splitting the sample by policy regime reveals a striking pattern (Figure 2). During rate hikes, the target shock dominates sentiment; during rate cuts, both target and path shocks matter; during unchanged-rate meetings, neither shock predicts sentiment. This regime-dependence is the central empirical finding of this paper and is robust across all three measurement approaches (Figure 3).')
...
P('Monetary policy shocks and asset prices. Kuttner (2001) introduced the federal funds rate surprise as a measure of unexpected monetary policy, and Gürkaynak, Sack, and Swanson (2005b) decomposed this into target and path shocks. Bernanke and Kuttner (2005) showed that stock prices respond significantly to monetary policy surprises. Nakamura and Steinsson (2018) used high-frequency identification to estimate the real effects of monetary policy. Bauer and Swanson (2023) reassessed the identification of monetary policy shocks, emphasizing the importance of controlling for information effects. Our contribution is to link these shocks directly to the textual content of FOMC statements, providing a channel through which shocks affect expectations.')
P('Textual analysis of central bank communications. Loughran and McDonald (2011) developed the standard financial sentiment dictionary, which has been widely applied to FOMC statements (Tetlock, 2007; Gurkaynak et al., 2005a). Cieslak and Schrimpf (2019) introduced a monetary-policy-specific dictionary. Hansen and McMahon (2016) used textual analysis to study the informational content of monetary policy communications. Our contribution is to demonstrate that the choice of dictionary has first-order implications for the results, and that the abbreviated LM dictionary produces misleading findings due to its positivity bias.')
P('LLM-based analysis of monetary policy. Chen, Granville, and Matousek (2026) demonstrate that GPT-4 can decode FOMC materials into four topics including forward guidance, while GPT-3.5 misses 97% of forward guidance content. Gambacorta et al. (2024) introduce CB-LMs — open-weight models retrained on central bank corpora. Fernández-Fuertes (2026) demonstrates that a multi-agent LLM framework can construct narrative monetary policy surprises. Our contribution is to provide a benchmark showing that even simple dictionary-based approaches can detect regime-dependent patterns when the correct dictionary is used.')
...
H('2.4 Asset Returns')
P('We construct cumulative abnormal returns (CARs) for 14 US banks and 5 Japanese banks around FOMC meeting dates, using a market model estimated over the [−250, −50] trading day window. For US banks, we use the CRSP value-weighted index as the market proxy; for Japanese banks, we use the Nikkei 225. The event window is [0, 1] trading days relative to the FOMC announcement. We also construct CARs around the release of FOMC minutes (typically 3 weeks after the meeting) to test for delayed information processing.')

H('2.5 Regime Classification')
...
P('For asset return regressions, we estimate:')
P('CAR_i,j = α_j + β_j × Target_i + γ_j × Path_i + δ_j × Sentiment_i + ε_i,j', italic=True)
P('where CAR_i,j is the cumulative abnormal return for bank j around meeting i. We use panel regressions with bank and time fixed effects, clustering standard errors at the bank level.')
...
P('Table 3 reports the regime-dependent regressions. The results are strikingly different from the full sample and reveal a clear pattern of differential sensitivity across policy regimes (Figure 2).')
...
P('The negative coefficient on the LM% target shock (β_T = −0.488) indicates that larger unexpected rate hikes are associated with more negative LM% scores — i.e., more negative financial sentiment. This is consistent with the interpretation that the LM dictionary captures the market-unsettling aspect of unexpected tightening. The positive coefficient in the CB specification (β_T = 0.011) reflects the different scale and construction of the CB score, where positive values indicate hawkish language.')
...
P('The convergence of three fundamentally different measurement approaches — a general-purpose financial dictionary (LM), a domain-specific dictionary (CB), and a large language model — on the same regime-dependent pattern rules out the possibility that the result is a methodological artifact of any single approach. This convergence is particularly striking given the very different construction of the three measures: the LM dictionary uses a general financial word list, the CB dictionary uses domain-specific terms and phrases, and the LLM uses contextual understanding of the full statement text.')

H('3.4 Forward Guidance Dimension')
P('The LLM classification provides information that the CB dictionary cannot. The forward guidance dimension — which the CB dictionary cannot distinguish from other policy-relevant text — shows that 31% of statements contain easing forward guidance, 15% contain tightening forward guidance, and 31% contain no forward guidance at all. The remaining 23% contain neutral forward guidance.')
...
P('The abbreviated LM dictionary (116 positive + 213 negative terms) produces LM% scores that are positive for 95% of FOMC statements, with a mean of 3.67%. The full dictionary (347 positive + 2,345 negative terms) produces scores that are positive for 76% of statements, with a mean of 0.24%. The correlation between the two measures is −0.27: they point in opposite directions (Figure 1). The root cause is that the abbreviated dictionary omits most negative financial terms, creating an upward bias that makes nearly all statements appear hawkish.')
...
P('This finding has a clear interpretation: the Kuttner surprise is the genuine driver of bank CARs, and the abbreviated LM% was acting as a noisy proxy for the same information. The full-dictionary LM%, which properly measures net sentiment, adds no incremental information beyond the rate surprise itself. This is consistent with the interpretation that general financial sentiment — as captured by the LM dictionary — primarily reflects the rate decision, while domain-specific language — as captured by the CB dictionary — captures forward guidance that the rate surprise does not subsume.')
...
H('5.4 Statement–Minutes Divergence')
P('A unique feature of FOMC communication is the three-week lag between the statement release and the minutes publication. This creates a natural experiment: if the minutes contain information that the statement does not, we should observe a significant market reaction at the minutes release. Figure 5 shows the distribution of statement–minutes sentiment divergence.')
P('The average absolute divergence is 3.39 percentage points in LM% terms, and the correlation between statement and minutes sentiment is moderate (r = 0.52). The divergence is larger during periods of policy uncertainty (the GFC and the 2020 pandemic), suggesting that the FOMC uses the minutes to clarify ambiguous statements. The minutes-release CAR is statistically significant for some banks (PNC, SCHW), consistent with the interpretation that the minutes contain incremental information.')

H('5.5 Three-Document Sentiment Gradient')
P('Figure 6 shows the sentiment gradient across three FOMC documents: the statement, the minutes, and the transcript. The statement is the most hawkish document (mean LM% = 0.24%), the minutes are more balanced (mean LM% = −0.15%), and the transcript is the most dovish (mean LM% = −0.42%). This gradient reflects the increasing deliberative content as we move from the public-facing statement to the internal discussion.')
P('The three-document gradient provides additional evidence for the informational revelation channel. If the statement were purely an implementation device, we would expect no systematic difference between the statement and the minutes. The fact that the minutes are more dovish on average suggests that the FOMC deliberately uses hawkish language in the statement to manage expectations, while the more balanced minutes reveal the true range of views.')
...
T(['Data Source', 'β_T', 'p_T', 'R²'],
  ...
  'Table 8: Data Source Comparison (LM% as dependent variable)')
...
P('This information bound has important implications for the literature. Studies that find low R² when regressing sentiment on high-frequency shocks should not conclude that sentiment is uninformative — rather, the shocks themselves are incomplete measures of monetary policy. The Fernández-Fuertes result suggests that the next generation of monetary policy research should use LLM-based narrative surprises rather than derivatives-based measures.')

H('6.3 Text Cleaning Sensitivity')
P('We verify that our results are robust to the text extraction method. Scoring FOMC statements with and without the voting paragraph, and with different boilerplate removal strategies, produces sentiment scores with correlations above 0.90. The regime-dependent significance pattern is preserved across all extraction methods. The main effect of including the voting paragraph is to dilute the sentiment scores with neutral words, reducing the absolute value of both LM% and CB scores but not changing their sign or relative ranking.')

H('6.4 The Bauer-Swanson Critique')
P('Bauer and Swanson (2023) argue that high-frequency monetary policy surprises may be contaminated by information effects — i.e., they may reflect the Fed\'s private information about the economy rather than pure monetary policy shocks. This critique applies to both the target and path shocks used in our analysis. The Jarociński and Karadi (2020) decomposition, which separates monetary policy shocks from information shocks using the co-movement of interest rates and stock prices, would provide a cleaner identification strategy. We leave this extension for future work.')

H('6.5 Sample Period Sensitivity')
P('Our sample (2006–2022) spans multiple monetary policy regimes: the 2006–2007 tightening cycle, the Global Financial Crisis and zero lower bound period (2008–2015), the 2016–2018 normalization, and the 2020–2022 pandemic response. The regime-dependent results are robust to excluding the GFC period (2008–2009) and the pandemic period (2020–2022), suggesting that they are not driven by crisis-era dynamics.')

H('6.6 Sentiment Persistence')
P('Statement sentiment exhibits moderate persistence: the autocorrelation of CB scores at lag 1 is 0.42 (p < 0.001). This persistence is consistent with the interpretation that the FOMC adjusts its language gradually, rather than making sharp changes from meeting to meeting. Including lagged sentiment as a control reduces the R² of the shock regressions only slightly, suggesting that the shock effects are not simply capturing sentiment momentum.')

H('6.7 Kuttner Surprise in Basis Points')
P('An alternative specification uses the Kuttner surprise in basis points rather than the GSS target shock. The results are qualitatively similar: the Kuttner surprise is significant during rate hikes (t = −4.12) and rate cuts (t = 2.87), but not during unchanged-rate meetings (t = 0.34). The path shock remains significant only during rate cuts. This confirms that the regime-dependent pattern is not an artifact of the GSS decomposition.')

H('6.8 Inner Confidence and LLM Reliability')
P('Chen et al. (2025) propose using token-level softmax entropy as a measure of LLM "inner confidence" — an honest signal of model uncertainty. We compute inner confidence for our LLM classifications and find that it is negatively correlated with the ambiguity of the statement (r = −0.31): the LLM is less confident about statements that are inherently ambiguous. This suggests that LLM-based sentiment measures should be weighted by inner confidence in future work, with less confident classifications receiving lower weights.')

H('6.9 US–Japan Cross-Market Comparison')
P('We compare the response of US and Japanese bank stocks to FOMC surprises. US banks respond significantly to the target shock (average CAR = −0.45% per standard deviation), while Japanese banks do not (average CAR = −0.08%, not significant). This asymmetry is consistent with the portfolio rebalancing channel: US banks are directly exposed to US interest rate changes through their holdings of US Treasury securities, while Japanese banks are primarily exposed to Japanese interest rates.')

H('6.10 Forward Guidance Interaction')
P('We test whether the effect of sentiment on asset returns is moderated by the presence of forward guidance. The interaction term Sentiment × FG_strength is positive and significant for the NASDAQ index (β = 202.17, p = 0.041), suggesting that forward guidance amplifies the effect of sentiment on technology stock returns. However, the economic magnitude is small: a one-standard-deviation change in sentiment implies a 1.2 basis point change in returns — negligible. The large raw coefficient reflects the extremely small scale of the sentiment variable rather than a meaningful economic relationship.')

H('6.11 Ambiguity and Market Volatility')
P('We construct an ambiguity score for each FOMC statement based on the disagreement between the LM and CB dictionaries. Statements with high ambiguity (LM and CB disagree on direction) are associated with higher market volatility in the post-announcement window (VIX change = +0.89% for high-ambiguity vs. +0.12% for low-ambiguity statements, p = 0.034). This suggests that textual ambiguity is a source of market uncertainty that is not captured by the point-in-time surprise measures.')

# ==================== 7. DISCUSSION ====================
H('7. Discussion')

H('7.1 From Dictionaries to LLMs')
P('Our finding that the CB dictionary outperforms the LM dictionary has important implications for the sentiment analysis literature, but even the CB dictionary is a bag-of-words approach that cannot capture nuanced semantics. Three recent developments suggest a clear path forward.')
P('First, Chen, Granville, and Matousek (2026) demonstrate that GPT-4 can decode FOMC materials into four topics — economic assessment, risk assessment, forward guidance, and policy stance — while GPT-3.5 misses 97% of forward guidance content. This finding implies that the choice of LLM matters as much as the choice of dictionary, and that forward guidance — the dimension most relevant to our path shock results — requires sophisticated language understanding to detect.')
P('Second, Gambacorta et al. (2024) introduce CB-LMs — open-weight encoder-only models retrained on central bank corpora — that outperform general models in capturing monetary policy nuances while offering full reproducibility. For our analysis, replacing the CB dictionary with CB-LM embeddings could capture contextual meaning while preserving reproducibility — a key advantage over proprietary LLM APIs.')
P('Third, Fernández-Fuertes (2026) demonstrates that a multi-agent LLM framework can construct narrative monetary policy surprises that capture 81.5% more information than standard high-frequency measures. His approach — which processes Statements, Minutes, Beige Books, and press conferences — represents the most ambitious application of LLMs to monetary policy identification to date. The key insight is that LLMs can extract conditional expectations from Fed communications, yielding less noisy surprises than market-based measures.')
P('Yao and Chai (2025) propose an uncertainty-aware LLM framework that explicitly accounts for model uncertainty and variability in LLM outputs. This approach is particularly relevant for constructing confidence intervals around LLM sentiment scores, addressing a key concern about the reliability of LLM-based measures.')
P('Taken together, these developments suggest that the next generation of monetary policy communication research will use LLM-based measures — either as replacements for dictionary-based sentiment scores or as tools for constructing narrative shocks — rather than the bag-of-words approaches that have dominated the literature to date. Our finding that the CB dictionary outperforms the LM dictionary by an order of magnitude is a first step in this direction, but the Chen et al. (2026) result implies that even the CB dictionary captures only a fraction of the relevant semantic variation.')
...
refs = [
    ...
    'Chen, Z., et al. (2025). Inner confidence in large language models. NBER Working Paper No. 34965.',
...
H('D. Figures')
FIG('paper/figures/fig1_lm_bias.png', 'Figure 1: The LM Dictionary Positivity Bias')
FIG('paper/figures/fig2_regime_scatter.png', 'Figure 2: Regime-Dependent Sentiment–Shock Relationship')
FIG('paper/figures/fig3_r2_by_regime.png', 'Figure 3: Explained Variance by Regime and Sentiment Measure')
FIG('paper/figures/fig4_timeseries.png', 'Figure 4: Monetary Policy Shocks and Statement Sentiment, 2006–2022')
FIG('paper/figures/fig5_stmt_minutes.png', 'Figure 5: Statement–Minutes Sentiment Divergence')
FIG('paper/figures/fig6_three_document.png', 'Figure 6: Sentiment Gradient Across FOMC Documents')
FIG('paper/figures/fig7_kuttner_horse_race.png', 'Figure 7: Kuttner Surprise Horse Race — Impact of LM Dictionary Correction')

# SAVE
doc.save('paper/Words_Beyond_the_Rate_v12_1.docx')
